...
> **参考图字段映射注意事项**：grok、nano-banana 系列、imagen-4 的上游要求参考图使用 `files` 字段。下游统一用 `ref_images` 字段传参即可，后端会自动映射到上游的正确字段。grok 上游有三种互斥的参考图方式（优先级: `files` > `file_urls` > `ref_images`），后端会根据输入类型自动选择。
