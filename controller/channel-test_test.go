...
func TestBuildTestRequest_SeedreamImageDetection(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "seedream auto detected as image request with 1920x1920",
		...
		{
			name:         "Seedream mixed case auto detected as image request with 1920x1920",
		...
		{
			name:         "seedream-4-0-250828 auto detected as image request with 1920x1920",
		...
		{
			name:         "explicit image endpoint uses seedream 1920x1920",
		...
		{
			name:         "gpt-image remains detected as image request with 1024x1024",
			model:        "gpt-image-1",
			endpointType: "",
			channel:      openAIChannel,
			wantType:     "*dto.ImageRequest",
		...
		{
			name:         "dall-e explicit image endpoint remains 1024x1024",
			model:        "dall-e-3",
			endpointType: string(constant.EndpointTypeImageGeneration),
			channel:      openAIChannel,
			wantType:     "*dto.ImageRequest",
		...
		{
			name:         "imagen explicit image endpoint remains 1024x1024",
		...
		{
			name:         "nano-banana explicit image endpoint remains 1024x1024",
